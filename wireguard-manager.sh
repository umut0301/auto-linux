...
detect_xui_ports() {
    ...
    if command -v /usr/local/x-ui/x-ui &>/dev/null; then
        local login_port=$(/usr/local/x-ui/x-ui setting -show 2>/dev/null | grep -oE 'port:[0-9]+' | cut -d':' -f2)
        if [ -n "$login_port" ]; then
            xui_ports+=("$login_port")
            log_info "检测到 x-ui 登录端口: $login_port"
        fi
    fi
    
    # 检测 x-ui 节点端口（从 config.json 读取）
    local config_file="/usr/local/x-ui/bin/config.json"
    if [ -f "$config_file" ] && command -v jq &>/dev/null; then
        # 使用 jq 提取所有 inbounds 的端口
        local node_ports=$(jq -r '.inbounds[]?.port // empty' "$config_file" 2>/dev/null | sort -u)
            ...
            while IFS= read -r node_port; do
                if [ -n "$node_port" ] && [[ "$node_port" =~ ^[0-9]+$ ]]; then
                    ...
                    if [[ ! " ${xui_ports[@]} " =~ " ${node_port} " ]]; then
                        xui_ports+=("$node_port")
                        log_info "检测到 x-ui 节点端口: $node_port"
                    fi
                fi
            done <<< "$node_ports"
        fi
...
configure_firewall() {
    ...
    # 检测 x-ui 端口
    local xui_ports=($(detect_xui_ports))
    ...
    case $firewall_type in
        firewalld)
            ...
            if [ ${#xui_ports[@]} -gt 0 ]; then
                log_info "正在开放 x-ui 端口..."
                for xui_port in "${xui_ports[@]}"; do
                    # 开放TCP端口
                    if ! firewall-cmd --permanent --query-port="${xui_port}/tcp" &>/dev/null; then
                        firewall-cmd --permanent --add-port="${xui_port}/tcp" 2>/dev/null
                        log_success "已开放 x-ui 端口 ${xui_port}/tcp 🔓"
                    else
                        log_info "端口 ${xui_port}/tcp 已在firewalld中开放"
                    fi
                    # 开放UDP端口
                    if ! firewall-cmd --permanent --query-port="${xui_port}/udp" &>/dev/null; then
                        firewall-cmd --permanent --add-port="${xui_port}/udp" 2>/dev/null
                        log_success "已开放 x-ui 端口 ${xui_port}/udp 🔓"
                    else
                        log_info "端口 ${xui_port}/udp 已在firewalld中开放"
                    fi
                done
            fi
            ...
            if [ ${#xui_ports[@]} -gt 0 ]; then
                ...
                for xui_port in "${xui_ports[@]}"; do
                    ...
                    if ! ufw status | grep -q "${xui_port}/tcp"; then
                        ufw allow ${xui_port}/tcp comment 'x-ui' 2>/dev/null
                        log_success "已开放 x-ui 端口 ${xui_port}/tcp 🔓"
                    else
                        log_info "端口 ${xui_port}/tcp 已在ufw中开放"
                    fi
                    # 开放UDP端口
                    if ! ufw status | grep -q "${xui_port}/udp"; then
                        ufw allow ${xui_port}/udp comment 'x-ui' 2>/dev/null
                        log_success "已开放 x-ui 端口 ${xui_port}/udp 🔓"
                    else
                        log_info "端口 ${xui_port}/udp 已在ufw中开放"
                    fi
                done
            fi
            ...
            if [ ${#xui_ports[@]} -gt 0 ]; then
                ...
                for xui_port in "${xui_ports[@]}"; do
                    ...
                    if ! iptables -C INPUT -p tcp --dport $xui_port -j ACCEPT 2>/dev/null; then
                        ...
                        log_success "已添加iptables规则：开放 x-ui 端口 ${xui_port}/tcp 🔓"
                    else
                        log_info "端口 ${xui_port}/tcp 已在iptables中开放"
                    fi
                    # 开放UDP端口
                    if ! iptables -C INPUT -p udp --dport $xui_port -j ACCEPT 2>/dev/null; then
                        iptables -I INPUT -p udp --dport $xui_port -j ACCEPT
                        log_success "已添加iptables规则：开放 x-ui 端口 ${xui_port}/udp 🔓"
                    else
                        log_info "端口 ${xui_port}/udp 已在iptables中开放"
                    fi
                done
            fi
        ...
        nftables)
            ...
            if [ ${#xui_ports[@]} -gt 0 ]; then
                ...
                for xui_port in "${xui_ports[@]}"; do
                    # 开放TCP端口
                    if ! nft list chain inet filter input 2>/dev/null | grep -q "tcp dport $xui_port"; then
                        nft add rule inet filter input tcp dport $xui_port accept 2>/dev/null || \
                        nft insert rule inet filter input tcp dport $xui_port accept 2>/dev/null
                        log_success "已添加nftables规则：开放 x-ui 端口 ${xui_port}/tcp 🔓"
                    else
                        log_info "端口 ${xui_port}/tcp 已在nftables中开放"
                    fi
                    # 开放UDP端口
                    if ! nft list chain inet filter input 2>/dev/null | grep -q "udp dport $xui_port"; then
                        nft add rule inet filter input udp dport $xui_port accept 2>/dev/null || \
                        nft insert rule inet filter input udp dport $xui_port accept 2>/dev/null
                        log_success "已添加nftables规则：开放 x-ui 端口 ${xui_port}/udp 🔓"
                    else
                        log_info "端口 ${xui_port}/udp 已在nftables中开放"
                    fi
                done
            fi
...
setup_firewall() {
    ...
    if [ ${#xui_ports[@]} -gt 0 ]; then
        echo ""
        log_info "x-ui 端口已自动开放:"
        for xui_port in "${xui_ports[@]}"; do
            log_info "  - 端口 ${xui_port}/tcp 和 ${xui_port}/udp 🔓"
        done
    else
        log_info "未检测到 x-ui 安装，跳过 x-ui 端口配置"
    fi
